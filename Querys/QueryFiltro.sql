(Agent like 'Cisco CPN' and CauseType > 0 and Type < 4 and AlertGroup in ('BFD connectivity down', 'BFD neighbour loss', 'BGP link down', 'BGP neighbour loss', 'efp down' ,'lag down',
...
'Spanning-Tree-Topology-Change-Trap', 'vsi down', 'vtp notification prefix trap')) or (Agent like 'ADSL' and CauseType > 0 and Type < 4 and AlertGroup in ('The Memory Usage Is Too High', 
...
'The ADSL port activation rate fails to reach the rate threshold.', 'EventKeepAlive')) or (Agent like 'Alcatel 5620 SAM' and CauseType > 0 and Type < 4 and AlertGroup in ('ReachabilityProblem', 
...
'SdpBindingDown', 'TunnelDown', 'TunnelAdministrativelyDown')) or (Agent like 'Alcatel 5620 SAM CC' and CauseType > 0 and Type < 4 and AlertGroup in ('GenericInterfaceLinkDown', 'link down', 'LinkDown', 
'netw.PhysicalLink', 'PollerProblem', 'NodeRebooted', 'HandShaking', 'ReachabilityProblem')) or (Agent like 'ADVA' and CauseType > 0 and Type < 4 and AlertGroup in ('AIS', 'FLT', 'PSU', 'ALLSYNCREF', 
...
'RXJABBER', 'RX-JABBER')) or (Agent like 'RAD' and CauseType > 0 and Type < 4 and AlertGroup in ('Alarm indication signal (ais)', 'link down', 'Loss of frame (lof)', 'Loss of signal (los)', 'Loss of signal (los) at far end (los-fe)', 
...
'Module was removed')) or (Agent like '^BM' and  CauseType > 0 and Type < 4 and AlertGroup in ('Traffic Failure', 'Transport Link Failure', 'Transport Segment Failure', 'Ingress Failure', 'Resource Isolation',
